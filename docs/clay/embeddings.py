...
class EmbeddingsHandler:
    ...
    def load_data(
        ...
        with ProcessPoolExecutor() as executor:
            gdfs = list(
                tqdm(
                    executor.map(self.read_embeddings_file, self.files),
    ...
    def plot(
        ...
        if max_rows is not None and len(self.gdf) > max_rows:
            ...
            self.gdf["geometry"] = [
                Point(xy) for xy in zip(self.gdf["x"], self.gdf["y"])
            ]
            self.gdf = self.gdf.drop_duplicates(subset=["geometry"])
            print(
                f"Warning: {len(self.gdf)} is more than {max_rows} rows, sampling {max_rows} ({100 * max_rows / len(self.gdf):.3f}%) and converting to points for plotting"
            )

            ax = (
                self.gdf.sample(max_rows)
                .reset_index(drop=True)
                .plot(figsize=figsize, alpha=alpha, edgecolor=edgecolor, markersize=1)
            )
        else:
            ax = self.gdf.plot(
                figsize=figsize, alpha=alpha, edgecolor=edgecolor, markersize=1
            )
    ...
    def local_imgs(self, local_folder: Path):  # replace s3 url with local folder
        # check if local_folder exists
        assert local_folder.exists(), f"{local_folder} does not exist"
        # check if local_folder has images
        assert (
            len(list(local_folder.rglob("*.tif"))) > 0
        ), f"{local_folder} does not have any images"

        # Replace the URL with a local folder
        self.gdf["tif_local_path"] = (
            self.gdf["source_url"]
            .str.replace("s3://", str(local_folder) + "/")
            .apply(Path)
        )
        ...
        assert (
            self.gdf["tif_local_path"].notnull().any()
        ), f"No local image paths found on {local_folder}"
    ...
    def image_sample(
        ...
        if len(self.gdf["tif_local_path"].notnull()) <= n:
            print(
                f'Warning: {len(self.gdf["tif_local_path"].notnull())} is less than {n} rows, returning all available rows'
            )
