...
def run_benchmark(dataset_config):
    ...
    if dataset_config['task'] == 'segmentation':
        cmd.extend(['decoder=seg_upernet', 'preprocessing=seg_default', 'criterion=cross_entropy'])
    elif dataset_config['task'] == 'regression':
        cmd.extend(['decoder=reg_upernet', 'preprocessing=reg_default', 'criterion=mse'])
...
def main():
    ...
    for i, dataset_config in enumerate(DATASETS, 1):
        ...
        print(f"[{i}/{len(DATASETS)}] {dataset_config['name']}: {status} ({elapsed_min:.1f}m)")
    ...
    print(f"\n🏁 BENCHMARK COMPLETE")
    print(f"Success rate: {successful}/{len(DATASETS)} ({successful/len(DATASETS)*100:.1f}%)")
