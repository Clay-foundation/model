...
class Clay_Encoder(Encoder):
    ...
    def __init__(
        ...
        encoder_weights: Union[
            str, Path
        ] = "./pretrained_models/clay_v1.5.0_epoch-07_val-loss-0.1718.ckpt",
        ...
        **kwargs,
        ...
        if input_bands is None:
            input_bands = {"optical": ["B2", "B3", "B4", "B8A", "B11", "B12"]}
        ...
        try:
            ...
            if "state_dict" in checkpoint:
                ...
                if logger:
                    if missing_keys:
                        logger.warning(
                            f"Missing keys when loading Clay encoder: {missing_keys}"
                        )
                    if unexpected_keys:
                        logger.warning(
                            f"Unexpected keys when loading Clay encoder: {unexpected_keys}"
                        )
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> List[torch.Tensor]:
        ...
        waves = torch.tensor(
            [
                ...
                842.0,
                1610.0,
                2190.0,  # Approximate wavelengths for B2,B3,B4,B8A,B11,B12
            ],
            device=device,
        )  # [6] - Clay expects a 1D tensor
        ...
        patches_with_encoding = self.clay_encoder.add_encodings(
            patches, time, latlon, gsd
        )
        ...
        encoded_patches = encoded_patches.reshape(
