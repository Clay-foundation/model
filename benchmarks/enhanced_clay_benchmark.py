...
def run_enhanced_benchmark(dataset_config, working_dir):
    ...
    print(
        f"Expected Performance: {dataset_config.get('expected_miou', dataset_config.get('expected_mae', 'TBD'))}"
    )
    print(f"{'='*80}")
...
def extract_performance_metrics(stdout, config):
    ...
    try:
        ...
        for line in lines[-50:]:  # Check last 50 lines
            ...
            if config["task"] == "regression" and (
                "mae" in line or "mean_absolute_error" in line
            ):
                try:
                    import re
...
def main():
    ...
    print("🎯 Focus: Demonstrating Clay's unique multimodal SAR+Optical capabilities")
    print(f"📊 Testing {len(ENHANCED_DATASETS)} carefully selected datasets")
    print("⚡ Enhanced configuration for optimal Clay performance")
    print()
    ...
    for i, dataset_config in enumerate(sorted_datasets, 1):
        ...
        print(
            f"[{i}/{len(ENHANCED_DATASETS)}] {dataset_config['name']}: {status} ({elapsed_min:.1f}m)"
        )

        # Performance summary if available
        if result["success"] and result.get("performance_metrics"):
            metrics_str = ", ".join(
                [f"{k}: {v}" for k, v in result["performance_metrics"].items()]
            )
            print(f"   📊 {metrics_str}")
    ...
    print("=" * 80)
    print(
        f"📊 Success rate: {successful}/{len(ENHANCED_DATASETS)} ({successful/len(ENHANCED_DATASETS)*100:.1f}%)"
    )
    ...
    print(
        f"   💪 Total demonstration: {successful}/{len(ENHANCED_DATASETS)} Clay capabilities"
    )
